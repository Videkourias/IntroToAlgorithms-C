...
long fibonacciRe(int n){
	if(n==0 || n == 1){
		 return n; //Base cases of fibonacci sequence
	}
	else{
...
Input: An integer n where n >= 0
Output: Returns a positive long integer, num3, the nth element of the fibonacci sequence
*/
...
void divRe(int n, int m){
	n-=1; //Reduces n by one as this only pertains to integers less than n, also serves to remove need to decrement n when calling the function recursively
	if(n == 0){
		 return; //Base case, returns void if there are no more numbers to check
	}
	if(n % m == 0){
